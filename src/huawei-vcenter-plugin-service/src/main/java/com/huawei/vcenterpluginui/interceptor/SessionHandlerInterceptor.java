...
public class SessionHandlerInterceptor extends HandlerInterceptorAdapter {
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
    ...
    if ((request.getRequestURI().endsWith("/services/notification/systemKeepAlive")
      ...
      if (!StringUtils.hasLength(request.getParameter("vcenterUsername")) || !StringUtils
          .hasLength(request.getParameter("vcenterPassword"))) {
        response.getWriter()
    ...
    if (!IS_VCENTER_THUMBPRINT_LOADED) {
      try {
        ...
        LOGGER.error("Failed to add vCenter thumbprints");
      }
    }
    return true;
  }
