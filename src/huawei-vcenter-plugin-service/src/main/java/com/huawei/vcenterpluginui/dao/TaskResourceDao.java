...
public class TaskResourceDao extends H2DataBaseDao {
  ...
  public List<TaskResource> getTaskResource(String taskName) throws SQLException {
    ...
    try {
      ...
      LOGGER.error("Failed to get task resource: " + e.getMessage());
      throw e;
    } finally {
  ...
  public int saveTaskResource(TaskResource taskResource) throws SQLException {
    ...
    try {
      ...
      LOGGER.error("Failed to save task resource: " + e.getMessage());
      throw e;
    } finally {
  ...
  public int updateTaskResource(TaskResource taskResource) throws SQLException {
    ...
    try {
      ...
      LOGGER.error("Failed to update task resource: " + e.getMessage());
      throw e;
    } finally {
      closeConnection(con, ps, null);
    }
  ...
  public int updateTaskResourceNotExistDN(List<String> dnList, TaskResource taskResource)
      throws SQLException {
    ...
    if (dnList != null && dnList.size() > 0) {
      try {
        ...
        sql.append(
            "update HW_TASK_RESOURCE set  SYNC_STATUS = ? ,DEVICE_RESULT = ?,ERROR_DETAIL = ?,DEVICE_PROGRESS=?,ERROR_CODE=?, LAST_MODIFY_TIME = CURRENT_TIMESTAMP where HW_ESIGHT_TASK_ID = ? and DN not in (");
