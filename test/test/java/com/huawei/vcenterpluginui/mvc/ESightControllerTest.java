...
public class ESightControllerTest extends ContextSupported {
    ...
    public void saveESight() throws IOException, SQLException {
        ...
        PowerMockito.when(HttpRequestUtil.requestWithBody(Mockito.anyString(), Mockito.<HttpMethod>any(), Mockito
                .<MultiValueMap>any(), Mockito.anyString(), Mockito.<Class<Object>>any())).thenReturn(responseEntity);
...
//        deleteESight();
    ...
    public void deleteESight() throws SQLException, IOException {
        ResponseBodyBean responseBodyBean = servicesController.getESightList(null, "192.168.10.84",0,0);
        Collection<ESight> eSightList = (Collection) responseBodyBean.getData();

        for (ESight eSight : eSightList) {
            Assert.assertEquals("0", servicesController.deleteESight(null,"{ids:["+eSight.getId()+"]}", mockHttpSession).getCode());
        }
    }
    ...
    public static ESight newESight() {
        ESight eSight = new ESight();
        eSight.setHostIp("192.168.10.84");
        eSight.setAliasName("ForTest");
        eSight.setHostPort(32102);
        eSight.setLoginAccount("openApiUser");
        eSight.setLoginPwd("Simple.0");
        eSight.setSystemId("NMSinfo2");
        return eSight;
    }
